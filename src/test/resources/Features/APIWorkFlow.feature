Feature: API workflow for HRMS
  ...
  Scenario: create an employee using API call
    ...
    Then the employee id "Employee.employee_id" is stored as a global variable to be use for another call
  ...
  Scenario: retrieve an employee using API call
    ...
    Then the employee data we get having id "employee.employee_id" must match with globaly store id
    Then the retrieved data at "employee" object matches with the data of created employee
      | emp_firstname | emp_lastname | emp_middle_name | emp_gender | emp_birthday | emp_status | emp_job_title|
      | Gigi          | Manini       | m               | Male         | 1984-05-20   | Probation  | Tester       |

  @apijsonworkflow
  Scenario: create an employee using API call
    Given a request is prepared to create an employee with dynamic data "Gigi","Manini","m","M","1984-05-20","Probation","Tester"
    When a POST call is made to create an employee
    Then the status code for creating an employee is 201
    Then the employee contains key "Message" and value "Employee Created"
    Then the employee id "Employee.employee_id" is stored as a global variable to be use for another call
    ...
    Scenario: updating the employee
      ...
      Then the status code of updated an employee is 200